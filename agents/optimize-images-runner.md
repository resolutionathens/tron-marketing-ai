...
[ -e "$SKILL_DIR/scripts/optimize-images.sh" ] || SKILL_DIR="$(for d in ~/.claude/plugins/cache/*/*/*/skills/$name ~/.claude/plugins/marketplaces/*/skills/$name; do [ -e "$d/scripts/optimize-images.sh" ] && echo "$d"; done | sort -V | tail -1)"
