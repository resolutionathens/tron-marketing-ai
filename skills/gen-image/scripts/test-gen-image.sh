...
[ -f "$logo" ] || logo="$(find ~/.claude/plugins/cache ~/.claude/plugins/marketplaces ~/.codex/plugins/cache ~/.codex/plugins/marketplaces "$HOME/Library/Application Support/tron-os/tron-releases/versions" -maxdepth 6 -type f -path "*/skills/md-to-pdf/facilitron-logo.png" 2>/dev/null | sort -V | tail -1 || true)"
