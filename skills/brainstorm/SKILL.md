...
[ -e "$SKILL_DIR/scripts/brainstorm.sh" ] || SKILL_DIR="$(for d in ~/.claude/plugins/cache/*/*/*/skills/$name ~/.claude/plugins/marketplaces/*/skills/$name; do [ -e "$d/scripts/brainstorm.sh" ] && echo "$d"; done | sort -V | tail -1)"
