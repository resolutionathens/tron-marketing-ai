...
- For the retro, run `tools/git/token-usage.sh`, write the marker, headings, literal model ID, and token line to a unique file from `RETRO_BODY="$(mktemp "${TMPDIR:-/tmp}/tron-retro-body.XXXXXX")"` (the template must end in `X` characters, with no suffix after them, or BSD/macOS `mktemp` returns that literal name and then fails `File exists`), then post it with `gh pr comment "<N>" --body-file "$RETRO_BODY"`.
