...
This is the canonical invocation for closing out a ticket — other skills reference this section
...
`tron:git-pushtoprod` already runs this automatically as part of promoting to production, for
repos that have a production branch. **Repos that ship by tag or PR-merge to their default branch
instead of promoting to production never run that stage**, so nothing transitions the ticket
automatically — a worker closing out the ticket (`tron:ship-ticket` stage 8 / `tron:close-worktree`)
must run this invocation explicitly before or during cleanup.
