...
The full command tables for the `gh` skill. The SKILL.md keeps the workflow and
judgment prose; this file is the lookup for the exact invocations. Each section
is a flat list of commands — copy the one you need and fill in `<N>` / `owner/repo`.
...
- Server-side (worktree-safe) merge commands
...
gh pr merge <N> --squash --delete-branch    # squash-merge + cleanup (NOT worktree-safe — see Merging in SKILL.md)
...
See SKILL.md "Merging a PR" for _why_ — this is the command reference.
