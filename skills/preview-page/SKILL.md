---
name: preview-page
description: Open a marketing-pages route in the cmux browser pane so Ian can visually inspect it. Spins up the Nuxt dev server on port 4001 if it isn't running, then adds a new browser surface in the browser pane pointing at the requested URL. Use this whenever Ian asks to "preview", "open in the browser", "show me the page", "check this in cmux", "load it up", "open that route", or anything similar after creating or editing a `pages/**/*.vue` file. Also trigger when he says "let's both look at it" or "pop it open" right after a page edit — that phrasing implies opening it in the cmux browser pane, not just printing the URL.
---

# Preview Page in cmux Browser

Goal: get a new or edited Nuxt page in front of Ian as fast as possible by opening it in the cmux browser pane, starting the dev server first if needed.
...
The cmux browser exposes a Playwright-style API on the surface. After a preview, you can verify your own work without bothering Ian. **Reach for these in order — stop at the first one that answers your question.**
...
When Ian gives you a Figma URL that points at the whole page frame (not a single section), don't dive into `get_design_context` — that returns one giant code dump and you lose the structure. Do this instead:
...
2. **Propose a numbered section list back to Ian** before building anything: node ID, name, your one-line guess at the component reuse (e.g. "SectionFeatureBestPractices in split mode + SectionCtaCaseStudy below"). Cross-check generic names like `Frame 1` by screenshotting the node. Wait for confirmation/corrections — section granularity in Figma is occasionally off (one Figma frame can hold two visually distinct sections, like a Best Practices grid plus a Case Study CTA).
3. **Loop section-by-section.** For each confirmed section: `get_design_context` on that node → identify reuse from `pages/solutions/**`, `pages/product/**`, `content/resources/toolkit/**` → implement → run the parity gate below. Don't batch — one section per round-trip, so Ian can redirect early if something's off.
4. **Commit cadence: wait until the page is done.** Don't commit between sections unless Ian asks. One commit per logical chunk (the whole page, or a tightly-scoped group) keeps history clean.

### Autonomous (Ian-is-away) variant — batch everything

When Ian explicitly hands off ("remote control / I'm walking away / ping me when ready"), the round-trip-per-section gate becomes the bottleneck. Use this parallel-batch shape instead — it walks a 9-section page in 2–3 minutes:
...
The trade-off vs. the interactive loop: zero mid-build HITL gates, so any divergence between Figma intent and what you built lands on Ian at review time. Mitigate by being more conservative with judgment calls — match Figma literally, list gaps for him to redirect.
...
4. **Enumerate visual diffs against the checklist below.** Write a numbered list. Mark each item: 🔴 real fix, 🟡 trade-off / open question, 🟢 framing artifact (e.g. site header not in Figma node, asset-content delta). Pay extra attention to direction/orientation items (image-left vs image-right, list order) — these are the easiest to miss and the most embarrassing.
   ...
   **How to read Diff mode (Ian's primary review tool):**
...
5. **STOP and request human-in-the-loop review of the enumeration.** Before applying any fix, share the list with Ian and wait for his confirmation/corrections. He sees the compare view; you don't have eyes on it the same way. He catches what you miss (layout direction, asset-vs-asset comparisons, "you said this matches but it doesn't"). **Do not skip this step to save a round-trip — the round-trip is the point.**
6. **Autonomous fix-and-verify loop, max 6 iterations.** Don't ping Ian between iterations. Run the loop yourself:
   ...
   for iter in 1..6:
     ...
     - if only 🟡/🟢 remain → exit loop, report to Ian for HITL review
     - if iter == 6 and 🔴 remain → exit loop, report what's still failing
     - if a 🔴 needs input you don't have (asset swap, copy decision, design choice) → reclassify as 🟡 escalation, exit loop, ask Ian
   ```
   ...
   Why no mid-loop pings: Ian is the QA gate, not the inner-loop reviewer. Iterating yourself is cheaper than a round-trip per fix; the trade-off only inverts if you're spinning on the same diff (the cap catches that).

   What counts as "needs Ian's input":
...
8. **Flag asset gaps early, don't ship around them.** If a Figma image is a composed mockup we don't have as a single asset, ask Ian for an export before iterating on smaller details. The wrong image dominates everything else visually.
9. **Prefer Ian's compare-view screenshots over re-pulling Figma.** When Ian pastes a screenshot of the compare page, my `Read` tool sees it at a higher resolution than a re-pulled Figma `get_screenshot` (because his is a much larger source PNG that downsamples to a still-larger image in my view). If he gives you a screenshot, USE THAT for the visual review rather than re-fetching from Figma.
...
Because of all this, prefer `scripts/responsive-shot.mjs` (headless, ignores pane visibility entirely) for self-verification screenshots, and reserve the in-pane `screenshot.sh` for when you specifically want Ian to see it live in his pane.
...
If you ship a blank to the compare view, Ian sees blank — that's a wasted round-trip.
...
- **Ask Ian to resize** the cmux browser pane and report what he sees. Use when you want his eyes on something subjective rather than a PNG.
...
Figure out the route path from what Ian gave you (or from what you just built):
...
- **Catch-all routes** (`pages/resources/news/[...slug].vue`) — Ian must give you the actual slug; you can't derive it from the file. Ask.

If Ian didn't name a page, check the conversation: a Vue file just got created or edited, that's almost certainly the one he wants to see.
...
A new browser tab opens in the browser pane. Don't focus it — Ian usually wants to stay in his current pane. The new tab is auto-selected within the browser pane, which is enough.

### 5. Tell Ian

One line, with the URL. He's about to look at it; he doesn't need a recap of what the page does.

### 6. (Optional) Self-check before handing off

If the change is non-trivial, run `cmux browser --surface $S errors list` and a `get text` for the most important element before saying "preview is up." Catching a broken h1 yourself is cheaper than catching it through Ian's eyes — and far cheaper than a screenshot round-trip.

## When things go sideways

- **No browser pane found.** Create one: `cmux new-pane --type browser --direction right --url "<full-url>"`. Tell Ian you split a new pane because no browser pane existed.
- **Port 4001 already busy by something else.** `lsof -ti:4001` returns a PID. If the process's command isn't `nuxt`/`node`, ask Ian before killing it.
...
- **Page 404s in the browser but the route file exists.** Nuxt needs a beat to pick up new files; if Ian just created the route, refresh after a second.
